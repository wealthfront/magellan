...
class Navigator(private val container: (Context) -> ScreenContainer?) : LifecycleAwareComponent() {
  ...
  private fun replace(nextNavigable: Navigable, navType: NavigationType) {
    navigate(FORWARD) {
      backStack.pop()
      backStack.push(NavigationEvent(nextNavigable, FORWARD, navType))
    }
  }
  ...
  private fun animateAndRemove(
    ...
    to?.whenMeasured {
      transition.animate(from, to, navEvent.navigationType, navEvent.direction) {
        if (context != null) {
  ...
  private fun showcurrentNavigable(direction: Direction): View? {
    ...
    when (currentState) {
      is Shown, is Resumed -> {
        ...
        containerView!!.addView(currentNavigable.view!!, indexToRemove)
      }
